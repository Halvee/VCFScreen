...
def main(ARGS = None):
    ...
    if args.gt_cnds != None: sample_cnds = VcfCnds(args.gt_cnds)
    ...
    for vcf_variant in cyvcf2_vcf.iterator(intervals):
        ...
        var_carriers = set()
        if gt_cnds != None: 
            for iid in samples_i.samples:
                iid_idx=samples_i.samples[iid].idx
                pheno = samples_i.samples[iid].pheno
                                
                ## is iid het or hom alt at site?
                sample_gt = vcf_variant.gt_types[iid_idx]
                if sample_gt not in set([1,2]): continue
                
                ## test if sample passes conditions in genotype cnds file
                if gt_cnds.test_gt(vcf_variant, iid_idx) == False: continue        
                
                var_carriers.add(iid)
                
        else:
            vargeno_screen = screens.vargeno_screen
            vargeno_carriers = vargeno_screen(vcf_variant, samples_i,
                                              min_coverage = args.min_coverage,
                                              min_perc_alt=args.min_perc_alt,
                                              max_perc_alt=args.max_perc_alt,
                                              homref_phredmin=args.homref_phredmin,
                                              het_phredmax=args.het_phredmax, 
                                              het_phredmin=args.het_phredmin,
                                              homalt_phredmin=args.homalt_phredmin,
                                              homalt_phredmax=args.homalt_phredmax)

        if len(vargeno_carriers) > 0:
            iids = list(vargeno_carriers)
            for iid in iids: 
                samples_i.samples[iid].varcounts["vargeno"] += 1
    ...
    vcf.close()
    samples_i.print_varcount_stats(var_types=["vargeno"],
                                   which="samples")
    return
