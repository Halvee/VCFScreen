...
'''
Filename : VCFscreen_parentalmosaic.py 
...
def main(ARGS = None):
    ...
    for vcf_variant in cyvcf2_vcf.iterator(intervals):
        ...
        if pro_cnds != None and transpar_cnds != None and ntranspar_cnds != None:
            for iid in samples_i.trios:
                ...
                ## is more likely transmitting parent GT equal to 0 or 1?
                if transpar_gt not in set([0,1]): continue
            ...
            mosaic_screen = screens.mosaic_screen
            res = mosaic_screen(vcf_variant, samples_i, iid_gts=set([1,2]),
    ...
    vcf.close()
    samples_i.print_varcount_stats(var_types=["mosaic"])
    return
